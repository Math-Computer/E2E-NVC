...
# if args.testclass=='B' or args.testclass=='C' or args.testclass=='D'or args.testclass=='E' :#yhy
#     path0='./data/test_sequence/'+args.testclass+'/'
#     files=os.listdir(path0)
#     for file in files:
#         path = path0+file
#         path_com = file + '_com_' + '_' + str(args.l) + '/'
#         path_bin = file + '_bin_' + '_' + str(args.l) + '/'
    ...
    with torch.no_grad():
        ...
        for batch_idx, input in enumerate(test_loader):
            ...
            for i in range(seqlen):
                ...
                clipped_recon_image, mse_loss, percep_loss, IQADict, warploss, interloss, bpp_feature, bpp_z, bpp_mv, bpp = net(inputframe, refframe)
...
def train(epoch, global_step):
    ...
    for batch_idx, input in enumerate(train_loader):
        ...
        clipped_recon_image, mse_loss, percep_loss, IQADict, warploss, interloss, bpp_feature, bpp_z, bpp_mv, bpp = net(input_image, ref_image, quant_noise_feature, quant_noise_z, quant_noise_mv)
        
        # tb = datetime.datetime.now()
        mse_loss, percep_loss, warploss, interloss, bpp_feature, bpp_z, bpp_mv, bpp = \
            torch.mean(mse_loss), torch.mean(percep_loss), torch.mean(warploss), torch.mean(interloss), torch.mean(bpp_feature), torch.mean(bpp_z), torch.mean(bpp_mv), torch.mean(bpp)
        distribution_loss = bpp
        if global_step < 500000:
            warp_weight = 0.1
        else:
            warp_weight = 0
        distortion = percep_loss + warp_weight * (warploss + interloss)
...
if __name__ == "__main__":
    ...
    train_dataset = DataSet("../../YHY/DVC/data/vimeo_septuplet/test.txt")
