...
class VideoCompressor(nn.Module):
    def __init__(self):
        ...
        # self.computeSSIM = SSIM()
        # self.computeMS_SSIM = MS_SSIM()
        # self.computeCW_SSIM = CW_SSIM()
        # self.computeGMSD = GMSD()
        # self.computeLPIPSvgg = LPIPSvgg()
        # self.computeDISTS = DISTS()
        # self.computeNLPD = NLPD()
        # self.computeFSIM = FSIM()
        # self.computeVSI = VSI()
        # self.computeVIFs = VIFs()
        # self.computeVIF = VIF()
        # self.computeMAD = MAD()
        
        if self.losstype == 'SSIM':
            self.losscompute = SSIM()
        if self.losstype == 'MS_SSIM':
            self.losscompute = MS_SSIM()
        if self.losstype == 'CW_SSIM':
            self.losscompute = CW_SSIM()
        if self.losstype == 'GMSD':
            self.losscompute = GMSD()
        if self.losstype == 'LPIPSvgg':
            self.losscompute = LPIPSvgg()
        if self.losstype == 'DISTS':
            self.losscompute = DISTS()
        if self.losstype == 'NLPD':
            self.losscompute = NLPD()
        if self.losstype == 'FSIM':
            self.losscompute = FSIM()
        if self.losstype == 'VSI':
            self.losscompute = VSI()
        if self.losstype == 'VIFs':
            self.losscompute = VIFs()
        if self.losstype == 'VIF':
            self.losscompute = VIF()
        if self.losstype == 'MAD':
            self.losscompute = MAD()
...
# distortion   
        ...
        if self.losstype == 'MSE':
            percep_loss = mse_loss
        else:
            self.losscompute.eval()
            percep_loss =  self.losscompute(recon_image, input_image)

        IQADict = {}
        # IQADict.update({'SSIM': self.computeSSIM(recon_image, input_image)})
        # IQADict.update({'MS_SSIM': self.computeMS_SSIM(recon_image, input_image)})
        # IQADict.update({'CW_SSIM': self.computeCW_SSIM(recon_image, input_image)})
        # IQADict.update({'GMSD': self.computeGMSD(recon_image, input_image)})
        # IQADict.update({'LPIPSvgg': self.computeLPIPSvgg(recon_image, input_image)})
        # IQADict.update({'DISTS': self.computeDISTS(recon_image, input_image)})
        # IQADict.update({'NLPD': self.computeNLPD(recon_image, input_image)})
        # IQADict.update({'FSIM': self.computeFSIM(recon_image, input_image)})
        # IQADict.update({'VSI': self.computeVSI(recon_image, input_image)})
        # IQADict.update({'VIFs': self.computeVIFs(recon_image, input_image)})
        # IQADict.update({'VIF': self.computeVIF(recon_image, input_image)})
        # IQADict.update({'MAD': self.computeMAD(recon_image, input_image)})

 
        # psnr = tf.cond(
        #     tf.equal(percep_loss, 0), lambda: tf.constant(100, dtype=tf.float32),
        #     lambda: 10 * (tf.log(1 * 1 / percep_loss) / np.log(10)))
...
# bit per pixel
        ...
        return clipped_recon_image, mse_loss, percep_loss, IQADict, warploss, interloss, bpp_feature, bpp_z, bpp_mv, bpp
